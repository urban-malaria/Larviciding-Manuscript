...
breeding_sites <- lav_bs_slum_wet
...
ggsave(paste0(LuDir, '/plots/', Sys.Date(), 'Sampled Pathways Between Fixed Breeding Sites(slum wetseason).pdf'), path_plot, width = 8, height = 6)
...
ggplot(path_summary, aes(x = Breeding_Site_Density)) +
  ...
  labs(title = "Distribution of Breeding Site Densities with ESW of 50m(slumwet)",
       x = "Density (sites/km²)",
       y = "Frequency") +
  theme_minimal()+
