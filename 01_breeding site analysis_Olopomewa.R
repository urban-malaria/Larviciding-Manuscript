...
breeding_sites <- lav_bs_formal_dry
...
path_plot <- ggplot() +
  # Add ward boundaries
  geom_sf(data = df_ib_o, fill = NA, color = "gray60", size = 0.5) +
...
ggsave(paste0(LuDir, '/plots/', Sys.Date(), 'Sampled Pathways Between Fixed Breeding Sites(formal dryseason).pdf'), path_plot, width = 8, height = 6)
...
ggplot(path_summary, aes(x = Breeding_Site_Density)) +
  ...
  labs(title = "Distribution of Breeding Site Densities with ESW of 50m(formaldry)",
       x = "Density (sites/km²)",
       y = "Frequency") +
  theme_minimal()+
