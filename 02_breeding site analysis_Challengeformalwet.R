...
breeding_sites <- lav_bs_formal_wet
...
path_plot <- ggplot() +
  # Add ward boundaries
  geom_sf(data = df_ib_c, fill = NA, color = "gray60", size = 0.5) +
...
ggsave(paste0(LuDir, '/plots/', Sys.Date(), 'Sampled Pathways Between Fixed Breeding Sites(formal wetseason).pdf'), path_plot, width = 8, height = 6)
...
x_pos <- quantile(path_summary$Breeding_Site_Density, 0.85) # 75th percentile for better placement
y_pos <- max(table(cut(path_summary$Breeding_Site_Density, breaks = 30))) * 0.9 # Just below the highest bar

ggplot(path_summary, aes(x = Breeding_Site_Density)) +
  geom_histogram(bins = 30, fill = "lightgreen", color = "black") +
  labs(title = "Distribution of Breeding Site Densities with ESW of 50m(formalwet)",
       x = "Density (sites/km²)",
       y = "Frequency") +
  theme_minimal()+
...
##Fixing the annotate output
# Check histogram ranges
range(path_summary$Breeding_Site_Density)

# Set safe x and y positions within those limits
x_pos <- mean(path_summary$Breeding_Site_Density)  # center horizontally
y_pos <- max(ggplot_build(ggplot(path_summary, aes(x = Breeding_Site_Density)) +
                            geom_histogram(bins = 30))$data[[1]]$count) * 0.9  # 90% of tallest bar height

# Now plot with annotate
ggplot(path_summary, aes(x = Breeding_Site_Density)) +
  geom_histogram(bins = 30, fill = "lightgreen", color = "black") +
  labs(title = "Distribution of Breeding Site Densities with ESW of 50m (formalwet)",
       x = "Density (sites/km²)",
